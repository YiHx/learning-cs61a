...
def floor_div(args):
    ...
    if args.rest == nil:
        return args.first
    new_first = args.first // args.rest.first
    new_args = Pair(new_first, args.rest.rest)
    return floor_div(new_args)
...
def eval_define(expressions):
    ...
    actual_value = calc_eval(value_expr)

    bindings[symbol] = actual_value
    return symbol
